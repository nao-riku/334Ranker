...
def interval3(until):
    while True:
        if until < datetime.datetime.now():
            if until <= datetime.datetime(start_now.year, start_now.month, start_now.day, 7, 32, 40):
...
def start():
    ...
    for i in range(len(times)):
        if start_now < times[i][0]:
            ...
            threading.Thread(target=interval3, args=(datetime.datetime(start_now.year, start_now.month, start_now.day, 7, 30, 51),)).start()
